...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        # We can split by paragraphs or just basic chunking for children
        child_paragraphs = [p.strip() for p in parent_text.split("\n\n") if p.strip()]
        current_child_text = ""
        for c_para in child_paragraphs:
            if len(current_child_text) + len(c_para) > child_size and current_child_text:
                children.append(Chunk(text=current_child_text.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                current_child_text = ""
            current_child_text += c_para + "\n\n"
    ...
    for section in sections:
        if re.match(r'^#{1,3}\s+', section):
            if current_content.strip():
                ...
                chunks.append(Chunk(text=text_content, metadata={**metadata, "section": current_header.strip(), "strategy": "structure"}))
